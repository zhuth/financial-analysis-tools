...
void lda_mle(lda_model* model, lda_suffstats* ss, int estimate_alpha)
{
  int k,d,w;
	...
	model->log_prob_k[k] = log(ss->class_data[k]) - log(ss->class_total);
    }
...
lda_suffstats* new_lda_suffstats(lda_model* model)
{
    ...
    ss->class_word_total = (double*)malloc(sizeof(double)*num_topics);
    ss->class_word = (double**)malloc(sizeof(double*)*num_topics);
    ss->class_data = (double*)malloc(sizeof(double)*num_topics);
    ...
    ss->class_doc_total = (double*)malloc(sizeof(double)*num_topics);
    ss->class_doc = (double**)malloc(sizeof(double*)*num_topics);
    for (i = 0; i < num_topics; i++)
    {
	ss->class_doc_total[i] = 0;
	ss->class_doc[i] = (double*)malloc(sizeof(double)*num_cited_docs);
...
void random_initialize_ss(lda_suffstats* ss, lda_model* model)
{
	...
	ss->class_data[k] = 1.0/num_topics + myrand();
	ss->class_total+=ss->class_data[k];
    }
    for (k = 0; k < num_topics; k++)
    {
...
lda_model* new_lda_model(int num_terms, int num_cited_docs, int num_topics)
{
    ...
    model->log_prob_k = (double*)malloc(sizeof(double)*num_topics);
    for (i = 0; i < num_topics; i++)
    {
    ...
    model->log_prob_d = (double**)malloc(sizeof(double*)*num_topics);
    for (i = 0; i < num_topics; i++)
    {
	model->log_prob_d[i] = (double*)malloc(sizeof(double)*num_cited_docs);
	for (j = 0; j < num_cited_docs; j++)
